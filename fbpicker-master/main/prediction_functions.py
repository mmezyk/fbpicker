...
class fbpicker():
    ...
    def amp_spectrum(data,dt,single=1):
        """ Calculates amplitude spectrum """
        if single==0:
    ...
    def kurtosis_skewness(data,hwin):
        """ Calculates higher order statistics: kurtosis & skewness """
        data_cp1=data.copy()
        data_cp2=data.copy()
        for k,l in enumerate(data):
    ...
    def trc_fgen_prefb(trc,dt,nspad=200,hwin=150,vlen=51):
        """ Constructs feature matrices for the pre-FB model """
        ...
        hwin2=50
        trc_kurtosis_skew=kurtosis_skewness(trc_norm_padded,hwin2)
        for i,j in enumerate(trc):
            ...
            ftrc=np.append(ftrc,norm(trc_kurtosis_skew[0])[np.int(nspad+fb/dt)-vlen+1:np.int(nspad+fb/dt)+1])
            ftrc=np.append(ftrc,norm(trc_kurtosis_skew[1])[np.int(nspad+fb/dt)-vlen+1:np.int(nspad+fb/dt)+1])
            ftrc=np.append(ftrc,1)
            output[i,:]=ftrc
        return output
    
    def trc_fgen_postfb(trc,dt,hwin=150,vlen=51):
        ...
        hwin2=50
        trc_kurtosis_skew=kurtosis_skewness(trc_norm,hwin2)
        for i,j in enumerate(trc):
            if i<len(trc)-vlen-1:
                ftrc=[]
                fb=i*dt
                ...
                ftrc=np.append(ftrc,norm(trc_kurtosis_skew[0])[np.int(fb/dt):np.int(fb/dt)+vlen])
                ftrc=np.append(ftrc,norm(trc_kurtosis_skew[1])[np.int(fb/dt):np.int(fb/dt)+vlen])
                ftrc=np.append(ftrc,1)
                output[i,:]=ftrc
        return output
    
    def trc_prep(m,ds,s,mode=2):
        """ Resizes a given matrix by down-sampling (Mode 1 - pre-FB, Mode 2 - post-FB)"""
    ...
    def predict(self):
        ...
        for i,j in enumerate(selected_trcs):
            trc_data_prefb=trc_fgen_prefb(self.dataset['data'][j][self.first_sample:self.first_sample+self.trc_slen],dt=self.dt)
            trc_data_postfb=trc_fgen_postfb(self.dataset['data'][j][self.first_sample:self.first_sample+self.trc_slen],dt=self.dt)
            trc_predictions=np.zeros((trc_data_prefb.shape[0],3))
            for k,l in enumerate((1,51,51)):
                trc_dataslice_prefb=trc_prep(trc_data_prefb,l,51,mode=1)
                trc_dataslice_postfb=trc_prep(trc_data_postfb,l,51,mode=2)
                model=self.models[k]
